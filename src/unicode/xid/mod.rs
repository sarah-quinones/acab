...
const fn bsearch_range_table(c: char, r: &[(char, char)]) -> bool {
	let mut begin = 0;
	let mut end = r.len();

	loop {
		// can't overflow, both < isize::MAX
		let mid = (begin + end) / 2;
		let (lo, hi) = r[mid];
...
pub const fn is_xid_start(c: char) -> bool {
	...
	('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') || (c > '\x7f' && bsearch_range_table(c, XID_START_TABLE))
}
...
pub const fn is_xid_continue(c: char) -> bool {
	...
	('a' <= c && c <= 'z')
		...
		|| ('0' <= c && c <= '9')
		|| c == '_'
		|| (c > '\x7f' && bsearch_range_table(c, XID_CONTINUE_TABLE))
}
